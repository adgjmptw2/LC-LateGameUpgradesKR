...
namespace MoreShipUpgrades.Misc
{
    internal static class CommandParser
    {
        ...
        public static readonly List<string> contractInfos = [
            "\n\n저희 시스템에서 시설 내 어딘가에 활성화된 PC가 감지되었습니다.\n해당 장치를 찾은 다음, 선박 터미널에서 장치의 IP 주소를 사용하여 무차별 대입 명령을 실행하여 로그인 자격 증명을 얻고, 그 다음 cd, ls, mv 명령을 사용하여 .db 파일을 찾으십시오(해당 폴더에서 `mv survey.db`를 입력).\n\n",
            "\n\n이 달에서는 저장벌레의 개체 수가 급증하고 공격적으로 변했다는 보고가 있습니다. 무슨 수를 써서라도 그들의 둥지를 파괴해야 합니다.\n\n",
            "\n\n5339번 승무원이 이 달에서 대원 한 명이 실종되어 남겨졌다고 보고했습니다. 실종된 대원을 치료하고 구조하려면 구급상자를 찾거나 가져와야 합니다.\n\n" ,
            "\n\n이 시설에서 영계의 비정상적인 활동이 보고되었습니다. \n의식 장소를 찾아 어떤 종류의 악마인지 확인하고, 터미널에 `악마 이름(DemonName)`을 입력하여 의식 지침을 받으세요. 의식 용품을 찾아 악마를 퇴치하십시오.\n\n" ,
            "\n\n신원 미상의 세력이 이 시설의 핵심 지점에 폭탄을 설치했습니다.\n여러분은 폭탄을 찾아내고 협력하여 해체해야 합니다.\n폭탄의 일련번호를 사용하여 `Lookup` 명령어를 실행하면 해체 지침을 얻을 수 있습니다.\n\n"
        ];
        ...
        {
            string displayText = "Late Game Upgrades_KR\n\n업그레이드를 보려면 `lategame store` 또는 `lgu`를 입력하세요.\n\n이 모드의 대부분은 `BepInEx/config/com.mine.lethalcompany.moreshipupgrades.cfg` 경로에 있는 설정 파일을 통해 구성할 수 있습니다..";
            displayText += "\n\n업그레이드와 관련된 모든 명령어를 보려면 `lategame commands` 또는 'lgc'를 입력하세요.";
            displayText += "\n\nlgu 저장 파일을 강제로 삭제하려면 `reset lgu`를 입력하십시오. (클라이언트 저장 파일만 삭제됩니다.)";
            displayText += "\n\n적용에 실패한 업그레이드를 다시 적용하려면 `load lgu`를 입력하십시오.";
            displayText += "\n\nMINE9289에 의해 번역되었습니다.";
            displayText += "\n\n";
        ...
        private static TerminalNode ExecuteInternsCommand(ref Terminal terminal, TerminalNode outputNode)
        {
            ...
            return DisplayTerminalMessage($"{player.playerUsername}이 다음으로 대체되었습니다:\n\n이름: {name}\n" +
                $"나이: {UnityEngine.Random.Range(19, 76)}\nIQ: {UnityEngine.Random.Range(2, 160)}\n" +
                $"관심 분야: {{관심 분야}}\\n\\n{{이름}}이 귀하의 위치로 발송되었습니다. 해당 내용을 숙지해 주시기 바랍니다.\n\n");
        }

...
namespace MoreShipUpgrades.UpgradeComponents.Items
{
    ...
    internal class Medkit : LategameItem, IDisplayInfo
    {
        ...
        private bool CanUseMedkit(int maximumHealth)
        {
            if (itemUsedUp)
            {
                hudManager.DisplayTip("더 이상 사용할 수 없습니다!", "이 구급상자에는 더 이상 물품이 없습니다!", true, false, "LC_Tip1");
                return false;
            }
        ...
        void UpdateMedkitUsagesLocal()
        {
            ...
            itemUsedUp = true;
            if (playerHeldBy == UpgradeBus.Instance.GetLocalPlayer()) hudManager.DisplayTip("더 이상 사용할 수 없습니다!", "이 구급상자에는 더 이상 물품이 없습니다!", true, false, "LC_Tip1");
        }

        public string GetDisplayInfo()
        {
            return $"MEDKIT - ${UpgradeBus.Instance.PluginConfiguration.MEDKIT_PRICE.Value}\n\n" +
                $"마우스 왼쪽 버튼을 클릭하여 {UpgradeBus.Instance.PluginConfiguration.MEDKIT_HEAL_VALUE.Value}만큼 체력을 회복하세요.\n" +
                $"{UpgradeBus.Instance.PluginConfiguration.MEDKIT_USES.Value}번 사용할 수 있습니다.";
        }

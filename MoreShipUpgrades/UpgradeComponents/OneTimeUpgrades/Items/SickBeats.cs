...
namespace MoreShipUpgrades.UpgradeComponents.OneTimeUpgrades.Items
{
    class SickBeats : OneTimeUpgrade, IUpgradeWorldBuilding
    {
        ...
        {
            SickBeatsUpgradeConfiguration config = GetConfiguration().SickBeatsUpgradeConfiguration;
            string txt = $"아픈 비트 - 활성화된 붐박스로부터 {config.Radius.Value} 단위 반경 내에 있는 플레이어는 다음과 같은 효과를 받습니다.\n\n";
            if (config.EnableSpeed.Value) txt += $"이동 속도가 {config.SpeedBoost.Value}만큼 증가합니다.\n";
            if (config.EnableDamage.Value) txt += $"입히는 피해량이 {config.DamageBoost.Value}만큼 증가합니다.\n";
            if (config.EnableDefense.Value) txt += $"받는 피해량에 {config.DefenseBoost.Value}를 곱합니다.\n";
            if (config.EnableStaminaRegen.Value)
            {
                txt += $"체력 회복량은 {config.StaminaRegenBoost.Value}로 곱해집니다.\n";
                if (config.ApplyStaminaDuringConsumption) txt += $"또한 시간이 지남에 따라 소모되는 스태미나에도 적용됩니다(승수 값은 {Mathf.Clamp(config.StaminaRegenBoost - 1f, 0f, 1f)}).\n";
            }
            return txt;
        }

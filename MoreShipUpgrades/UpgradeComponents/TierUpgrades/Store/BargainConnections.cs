...
namespace MoreShipUpgrades.UpgradeComponents.TierUpgrades.Store
{
    class BargainConnections : TierUpgrade, IUpgradeWorldBuilding
    {
        ...
        public override string GetDisplayInfo(int initialPrice = -1, int maxLevels = -1, int[] incrementalPrices = null)
        {
            ...
            const string infoFormat = "LVL {0} - {1} - 할인 가능한 아이템 수량을 {2}만큼 증가시킵니다.\n";
            return Tools.GenerateInfoForUpgrade(infoFormat, initialPrice, incrementalPrices, infoFunction, purchaseMode: GetConfiguration().BargainConnectionsConfiguration.PurchaseMode);
        }

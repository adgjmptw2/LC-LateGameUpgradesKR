...
namespace MoreShipUpgrades.UpgradeComponents.TierUpgrades.Player
{
    class BetterScanner : TierUpgrade, IUpgradeWorldBuilding
    {
        ...
        public static string GetBetterScannerInfo(int level, int price)
        {
            ...
            switch (level)
            {
                case 1: return string.Format("LVL {0} - ${1}: 일반 스캔 거리가 {2} 증가합니다. 함선 및 입구 스캔 거리가 {3} 증가합니다.\n", level, GetUpgradePrice(price, config.PurchaseMode), config.NodeRangeIncrease.Value, config.OutsideNodesRangeIncrease.Value);
                case 2: return string.Format("LVL {0} - ${1}: `scan hives`, `scan scrap`, `scan doors`, `scan enemies` 및 `scan player` 터미널 스캔 명령어가 잠금 해제됩니다.\n", level, GetUpgradePrice(price, config.PurchaseMode));
                case 3:
                    {
                        string result = string.Format("LVL {0} - ${1}: 스캐너는 벽을 통해 스크랩{2}을 스캔할 수 있습니다.\n", level, GetUpgradePrice(price, config.PurchaseMode), config.SeeEnemiesThroughWalls.Value ? " 그리고 적" : "");
                        result += "벌집 및 고철 명령어는 지도상에서 가장 가치 있는 벌집과 고철의 위치를 ​​표시합니다.\n";
                        return result;
                    }
            }
            return "";

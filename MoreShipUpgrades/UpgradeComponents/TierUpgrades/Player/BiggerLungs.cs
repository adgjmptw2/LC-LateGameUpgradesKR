...
namespace MoreShipUpgrades.UpgradeComponents.TierUpgrades
{
    class BiggerLungs : TierUpgrade, IUpgradeWorldBuilding
    {
        ...
        string GetBiggerlungsInfo(int level, int price)
        {
            ...
            sb.AppendFormat("레벨 {0} - ${1}: 스태미나 유지 시간이 {2} 단위 더 길어집니다\n", level, GetUpgradePrice(price, GetConfiguration().BiggerLungsConfiguration.PurchaseMode), infoFunction(level - 1));
            BiggerLungsUpgradeConfiguration config = GetConfiguration().BiggerLungsConfiguration;
            if (level >= config.StaminaRegenerationLevel) sb.Append($"스태미나 회복 속도가 {Mathf.FloorToInt(staminaRegenerationInfo(level) * 100f)}% 증가합니다.\n");
            if (level >= config.JumpReductionLevel.Value) sb.Append($"점프 시 소모되는 스태미나는 {Mathf.FloorToInt(costReductionInfo(level) * 100f)}%만큼 감소합니다.\n");
            return sb.ToString();
        }

...
namespace MoreShipUpgrades.UpgradeComponents.TierUpgrades.Player
{
    internal class NightVision : TierUpgrade, IPlayerSync, IUpgradeWorldBuilding
    {
        ...
        public static string GetNightVisionInfo(int level, int price)
        {
            ...
            return string.Format("LVL {0} - ${1}:\n배수 시간: {2}초 // 재생 시간: {3}초\n", level, GetUpgradePrice(price, config.PurchaseMode), drainTime, regenTime);
        }
        ...
        public override string GetDisplayInfo(int initialPrice = -1, int maxLevels = -1, int[] incrementalPrices = null)
        {
            ...
            stringBuilder.Append($"해당 아이템(야간 투시경)은 기본 에너지 소모 시간이 {drain}초이고, 에너지가 완전히 충전되는 데 걸리는 시간은 {regen}초입니다.\n\n");

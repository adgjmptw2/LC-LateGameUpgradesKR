...
namespace MoreShipUpgrades.UpgradeComponents.TierUpgrades.Items.Zapgun
{
    internal class AluminiumCoils : TierUpgrade, IUpgradeWorldBuilding
    {
        ...
        string GetAluminiumCoilsInfo(int level, int price)
        {
            ...
            sb.Append($"LVL {level} - {GetUpgradePrice(price, GetConfiguration().AluminiumCoilConfiguration.PurchaseMode)}: 잽 건 업그레이드:\n");
            sb.Append($"- 잽 건의 사거리를 {rangeInfo(level - 1) / 13f * 100f:F0}%만큼 증가시킵니다.\n");
            sb.Append($"- 기절 시간이 {stunTimerInfo(level - 1)}초만큼 증가했습니다.\n");
            sb.Append($"- 미니게임의 난이도를 {difficultyInfo(level - 1):F0}% 감소시킵니다.\n");
            sb.Append($"- 전기총의 재사용 대기시간을 {cooldownInfo(level - 1):F0}% 감소시킵니다.\n");
            sb.Append('\n');
            return sb.ToString();
        }
